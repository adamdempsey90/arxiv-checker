...
class Paper():
    ...
    def __str__(self):
        ...
        maxlen = max([len(self.title),
            len(self.link),
            len(authstr)]) + len(pad_left) + len(pad_right)

        border = ''.join(['%']*maxlen)
        spacer = pad_left + ''.join([' ']*(maxlen-len(pad_left)-len(pad_right))) + pad_right
        strbody = '\n\n' + \
                border + \
                '\n' + spacer + \
                '\n' + pad_left + self.title + ''.join([' ']*(maxlen -len(self.title) - len(pad_left) - len(pad_right)))+ pad_right + \
                '\n' + pad_left + self.link + ''.join([' ']*(maxlen -len(self.link) - len(pad_left) - len(pad_right))) + pad_right + \
                '\n' + pad_left + authstr + ''.join([' ']*(maxlen -len(authstr) - len(pad_left) - len(pad_right)))+ pad_right + \
                '\n' + spacer + \
                '\n' + border + '\n'
        return strbody
...
def check_authors(arxiv_names, author_filename):
    ...
    author_filename = text file that contains the names of the authors you want to check for.
    ...
    for arxiv_name in arxiv_names:
        ...
        authors_t = [entry.find_next('div', {'class':'list-authors'}).text.split(':')[-1].strip().split(', \n') for entry in entries]
        handles=[[val['href'].split('+')[-1].split('/')[0].lower() for val in entry.find_all('a')] for entry in entries]

        authors = authors + [ dict(zip(h,a)) for h,a in zip(handles, authors_t) ]

        titles = titles + [entry.find_next('div', {'class':'list-title'}).text.split(':')[-1].strip() for entry in entries]

        # Arxiv numbers are before the <dd> tag
        numbers_t = soup.find_all('span', {'class':'list-identifier'})
        numbers = numbers + [re.findall("\d+\.\d+", num.text)[0] for num in numbers_t]
    ...
    id_list = [(line.split(',')[0].replace('-','_').strip() + '_' + line.split(',')[1].strip()[0]).lower() for line in author_list]

    papers = []
    for person,id_t  in zip(first_names, id_list):
        for auths,title,number in zip(authors, titles,numbers):
            if id_t in auths.keys():
                # Check that the first name matches to avoid non-unique identifiers
                if person in auths[id_t].lower():
                    if number not in [paper.number for paper in papers]:
                        ...
                        for i,paper in enumerate(papers):
                            # Make sure we don't add the same author twice to the same paper
